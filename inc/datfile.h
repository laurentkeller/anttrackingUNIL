...
class DatFile{
	...
	public:
		...
		/**\brief Copy an existing datfile
		 * \param obvious
		 * \return True if copying was succesful, false otherwise
		 */
		bool copy(string source_filename, string dest_filename);
		...
		/**\brief Writes a single tag to the file at specific frame
		 * \param frame location
		 * \param tag data
		 * \return True if write successful, false if not
		 */
		bool write_tag(unsigned int frame, tag_pos &tag);
